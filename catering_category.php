...
if (isset($_POST["edit_name"]) AND !empty($_POST["edit_name"])) {
    CateringCategoryTable::update_category_name($_POST["edit_name"], $_POST["edit_id"]);
}
if(isset($_POST["delete_id"])) {
    CateringCategoryTable::remove_category($_POST["delete_id"], $_SESSION["date"]);
}
...
<body>
    <div class="category_main font_open_sans">
        <div class="div_category">
            ...
            <ul class="category_list" id="category_list">
                <?php $result = CateringCategoryTable::get_categories($_SESSION["date"]) ?>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <li id="<?php echo $row['id']?>" class="list_category_li" onclick=categorySelect(this)>
                        <span><?php echo $row["name"]?></span>
                        <form action="catering_category.php" method="post">
        ...
        <div class="list_container" id="list_container">
            ...
            <div class="div_item_list">
                ...
                <div class="div_list">
                    ...
                    <?php $result = CateringItemTable::get_uncategorized_items($_SESSION["date"]); ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
    ...
    <form action="catering_category.php" method="post" id="add_form">
        <input type="hidden" name="new_name" id="new_name">
    </form>
    <form action="catering_category.php" method="post" id="edit_form">
...
<script>
    function categorySelect(obj) {
        ...
        $.post("jq_ajax.php", {getCateringCategorizedItems:"", categoryName: categoryName, date: date}, function(data,status){
            ...
            $("#categorized_list").sortable({
                ...
                receive: function(event, ui) {
                    ...
                    $(this).children(".selected").each(function(){
                        $.post("jq_ajax.php", {UpdateCateringItemsCategory: "", itemName: $(this).attr("item-name"), categoryName: categoryName});
                    });
                },
                update: function(event, ui) {
                    ui.item.after(ui.item.data('multidrag')).remove();
                    var itemIds = $(this).sortable('toArray');
                    $.post("jq_ajax.php", {UpdateCateringItemOrder: "", itemIds: itemIds});
                }
    ...
    function addCategory() {
        ...
        $.post("jq_ajax.php", {addCateringCategory: "", name: name, date: $("#session_date").val()}, function(data) {
            switch (data) {
                case "category exists":
                    ...
                    break;
                case "category added":
                    // $.post("jq_ajax.php", {UpdateCateringCategoryOrder: "", categoryIds: ids});
                    $("#add_form").submit();
                    break;
            } 
        });
    ...
    $(document).ready(function() {
        ...
        $("#uncategorized_list").sortable({
            ...
            receive: function(event, ui) {
                $(this).children(".selected").each(function() {
                    $.post("jq_ajax.php", {UpdateCateringItemsCategory: "", itemName: $(this).attr("item-name"), categoryName: null});
                });
            }
        });
        ...
        $("#cat_item_sort").click(function() {
            alertify.confirm("Sort Items alphabetically?", function() {
                ...
                $.post("jq_ajax.php", {UpdateCateringItemOrder: "", itemIds: ids});
            });
        });
    });
</script>
